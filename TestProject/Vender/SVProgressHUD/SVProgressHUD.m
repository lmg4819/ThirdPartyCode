    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if (strongSelf) {
            ...
            if (progress >= 0) {
                ...
                if (strongSelf.backgroundRingView) {
           ...
           // Fade in delayed if a grace time is set
            if (self.graceTimeInterval > 0.0 && self.backgroundView.alpha == 0.0) {
...
-(UIImageView *)imageView
{
    ...
    if (!_imageView) {
        _imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, _imageViewSize.width, _imageViewSize.height)];
    }
...
{
    if (!_isInitializing) _defaultStyle = defaultStyle;
}
+(void)setDefaultMaskType:(SVProgressHUDMaskType)maskType
{
    [self sharedView].defaultMaskType = maskType;
}
-(void)setDefaultMaskType:(SVProgressHUDMaskType)defaultMaskType
{
    if (!_isInitializing) _defaultMaskType = defaultMaskType;
}
+(void)setDefaultAnimationType:(SVProgressHUDAnimationType)type
{
    [self sharedView].defaultAnimationType = type;
}
-(void)setDefaultAnimationType:(SVProgressHUDAnimationType)defaultAnimationType
{
    if (!_isInitializing) _defaultAnimationType = defaultAnimationType;
}
...
{
    if (!_isInitializing) _backgroundColor = backgroundColor;
}
...
{
    if (!_isInitializing) _foregroundColor = foregroundColor;
}
...
{
    if (!_isInitializing) _infoImage = infoImage;
}
...
{
    if (!_isInitializing) _successImage = successImage;
}
...
{
    if (!_isInitializing) _errorImage = errorImage;
}
...
-(void)setMinimumDismissTimeInterval:(NSTimeInterval)minimumDismissInterval
{
    if (!_isInitializing) _minimumDismissTimeInterval = minimumDismissInterval;
}
+(void)setMaximumDismissTimeInterval:(NSTimeInterval)interval
{
    [self sharedView].maximumDismissTimeInterval = interval;
}
-(void)setMaximumDismissTimeInterval:(NSTimeInterval)maximumDismissInterval
{
    if (!_isInitializing) _maximumDismissTimeInterval = maximumDismissInterval;
}
+(void)setFadeInAnimationDuration:(NSTimeInterval)duration
{
    [self sharedView].fadeInAnimationDuration = duration;
}
-(void)setFadeInAnimationDuration:(NSTimeInterval)fadeInAnimationDuration
{
    if (!_isInitializing) _fadeInAnimationDuration = fadeInAnimationDuration;
}
+(void)setFadeOutAnimationDuration:(NSTimeInterval)duration
{
    [self sharedView].fadeOutAnimationDuration = duration;
}
-(void)setFadeOutAnimationDuration:(NSTimeInterval)fadeOutAnimationDuration
{
    if (!_isInitializing) _fadeInAnimationDuration = fadeOutAnimationDuration;
}
...
{
    if (_isInitializing) _viewForExtension = viewForExtension;
}
...
{
    [self sharedView].hudView.layer.borderColor = (__bridge CGColorRef _Nullable)(color);
}
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:SVProgressHUDDidReceiveTouchEventNotification object:nil userInfo:[self notificationUserInfo]];
...
-(void)fadeIn:(id)data
{
    ...
    if (self.backgroundView.alpha != 1.0f) {
        ...
        if (self.fadeInAnimationDuration > 0) {
            [UIView animateWithDuration:self.fadeInAnimationDuration delay:0 options:(UIViewAnimationOptions) (UIViewAnimationOptionAllowUserInteraction | UIViewAnimationCurveEaseIn | UIViewAnimationOptionBeginFromCurrentState) animations:^{
                animationsBlock();
...
-(void)fadeInEffects
{
    if(self.defaultStyle != SVProgressHUDStyleCustom) {
        ...
        // This will make everytfrontWindowhing more readable. See the following for details:
